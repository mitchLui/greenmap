...
from .internal.tier_scooters import TierScooterService
...
tiers_service = TierScooterService("", "TIERAPIKEY")
...
async def get_transport(request: TransportRequest = Depends()):
    try:
        ...
        with ThreadPoolExecutor(max_workers=6) as e:
            ...
            futures.append(e.submit(voi_service.get_vehicles, request.lat, request.long, request.radius))
            futures.append(e.submit(tiers_service.get_scooters, request.lat, request.long, request.radius))
        ...
        voi_data = futures[4].result()
        tiers_data = futures[5].result()
        data = {
